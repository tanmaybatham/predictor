...
def getGraphAndTable(df, input_data, input_data2, customerName, productName, isJsonRequest):
    ...
    layout = go.Layout(
        title='Correlation as per ISV Assumptions'
    )
    ...
    trace1 = go.Scatter(x=my_df['Product Name'], y=my_df['Prediction Score(Based on Actual Usage)'], mode='lines', name='Prediction Score(Based on Actual Usage)')
    trace2 = go.Scatter(x=my_df['Product Name'], y=my_df['Prediction Score(Based on ISV Assumption)'], mode='lines', name='Prediction Score(Based on ISV Assumption)')
    trace3 = go.Scatter(x=my_df['Product Name'], y=my_df['Actual Score'], mode='lines', name='Actual Score')


    # Create the layout
    layout1 = go.Layout(title='Usage Comparison',width=1350, xaxis=dict(title='Product Name'), yaxis=dict(title='Usage Score'))

    # Create the figure object and add the traces and layout
    fig_graph1 = go.Figure(data=[trace1, trace2,trace3], layout=layout1)

    trace1_cust = go.Scatter(x=my_df2['Customer Name'], y=my_df2['Prediction Score(Based on Actual Usage)'], mode='lines',
                             name='Prediction Score(Based on Actual Usage)')
    trace2_cust = go.Scatter(x=my_df2['Customer Name'], y=my_df2['Prediction Score(Based on ISV Assumption)'], mode='lines',
                             name='Prediction Score(Based on ISV Assumption)')
    trace3_cust = go.Scatter(x=my_df2['Customer Name'], y=my_df2['Actual Score'], mode='lines', name='Actual Score')

    # Create the layout
    layout2 = go.Layout(title='Usage Comparison',width=1350, xaxis=dict(title='Customer Name'), yaxis=dict(title='Usage Score'))
